...
当前引擎输出的说话人标签附着在调用方提供的规范化转写句段上，评测预测时间范围因此沿用这些句段的起止时间。当前 DER/JER 观察值用于衡量固定转写句段上的自然人归属与拒识，不证明独立语音活动检测、精确说话人边界或完整端到端 diarization。参考标签文字按可置换的录音内自然人编号处理，不解释角色名称语义。

当前人数观察值按最终至少出现在一个非 `unknown` 句段上的标签数计算，不包含引擎已经建立但未用于最终句段归属的簇。监督验收需要另行核对结果中的全部实际建簇数；在评测器修正前，不得只用自动人数观察值裁决过度聚类。
